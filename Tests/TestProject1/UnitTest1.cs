...
public class Tests
{
    ...
    public void TestSequence1(int step)
    {
        ...
        builder = step switch
        {
            1 => BuildImplementationType(),
            2 => BuildImplementationInstance(),
            _ => BuildImplementationFactory()
        };
        host = builder.Build();
        ...
        host = builder.Build();
        LifetimeObserver lo = host.Services.GetRequiredService<LifetimeObserver>();
        foreach (Type type in lo.GetTracedTypes())
        {
            counts.Add(type, 0);
        }
        lo.LifetimeEventOccured += (s, e) =>
        {
        ...
        for (int i = 0; i < requests.Count; ++i)
        {
            ...
            {
                serviceScope = host.Services.CreateScope();
                Assert.That(results[i], Is.EqualTo(0));
            }
        }
        foreach(var it in counts)
        {
